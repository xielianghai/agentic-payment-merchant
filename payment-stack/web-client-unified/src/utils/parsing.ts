import type {A2APart, AgentArtifactData, ErrorArtifact, ImmediateCheckoutRequest, InventoryOptionsArtifact, MandateRequest, MonitoringStatus, Part, ProductPreviewUnavailable, PurchaseComplete, ToolCallArtifact} from '../types';
...
/** Strip one or more structured artifact JSON blobs (and optional ```json fences). */
export function stripAgentArtifactJson(
    text: string,
    ...typeVals: string[]
    ): string {
  let out = text;
  for (const typeVal of typeVals) {
    out = removeArtifactJsonFromText(out, typeVal);
  }
  return out;
}
...
/**
 * Searches for and returns the highest-priority structured artifact in message
 * parts (mandate_request wins over product_preview_unavailable in same turn).
 */
export function parseMainArtifactData(
  ...
  const normalizeMain = (
    ...
    if (type === 'product_preview_unavailable') {
      return normalizeProductPreviewUnavailable(raw) as AgentArtifactData;
    }
    if (type === 'immediate_checkout_request') {
      return normalizeImmediateCheckout(raw) as AgentArtifactData;
    }
    return raw as unknown as AgentArtifactData;
  };

  let best: {priority: number; data: AgentArtifactData}|undefined;

  const consider = (type: string, raw: Record<string, unknown>) => {
    const priority = mainTypes.indexOf(type);
    if (priority < 0) return;
    const data = normalizeMain(type, raw);
  ...
  for (const part of parts) {
    if (part.kind === 'data') {
      ...
      if (d.type && typeof d.type === 'string' && mainTypes.includes(d.type)) {
        consider(d.type, d as Record<string, unknown>);
      }
    } else if (part.kind === 'text') {
      for (const t of mainTypes) {
        const extracted = extractJsonFromText(part.text, 'type', t);
        if (extracted) {
          consider(t, extracted as Record<string, unknown>);
        }
      }
    }
  }
