...
export interface PurchaseComplete {
  ...
  /** Closed mandate chain ids — backend scheduler includes these for Mandates tab. */
...
export interface MandatesSigned {
  ...
  /** SD-JWT token (display) or mandate id when fetch failed. */
  open_checkout_mandate: string;
  open_payment_mandate: string;
  /** Persisted mandate ids for backend scheduler (:8105). */
  open_checkout_mandate_id?: string;
  open_payment_mandate_id?: string;
...
export type AgentArtifactData =|MandateRequest|PurchaseComplete|ErrorArtifact|
    ...
    MandateChainsFetched|ImmediateCheckoutRequest|Record<string, unknown>;
