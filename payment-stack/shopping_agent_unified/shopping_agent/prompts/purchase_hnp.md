...
- Call **get_ap2_session_config** first. For **card**, pass `vi_l2_credential_id` and `vi_l3_credential_id` from session state or the payment presentation step to **issue_payment_credential**, along with `amount_cents` and `currency`. For **x402**, call **create_x402_wallet_sign_session** then **wait_for_x402_wallet_signed** before **issue_payment_credential**. Use **issue_payment_credential** with `payment_method` and `presence_mode="hnp"` from session, plus `payment_mandate_chain_id`, `payment_nonce`, `open_checkout_hash`, `checkout_jwt_hash`.
