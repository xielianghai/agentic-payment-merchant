...
export function InventoryOptionsCard({
  ...
  const isOutOfStockHpFlow =
      hpFlow && !inventory.matches.some((m) => m.available);
  ...
  return (
    <div className="msg-agent inventory-options-container">
      ...
      {triggerCurl && isOutOfStockHpFlow && (
        <TriggerCurlBox
          curl={triggerCurl}
      ...
      {isOutOfStockHpFlow && onChoose && (
        <ActionChoicesCard
          options={[
            {label: 'Buy now again', value: 'buy now'},
            {label: 'Wait / re-check', value: 'wait and re-check availability'},
          ]}
          onChoose={onChoose}
        />
      )}
